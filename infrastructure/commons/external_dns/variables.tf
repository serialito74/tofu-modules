...
variable "zone_id_filter" {
  type        = string
  description = "The Route53 hosted zone ID for ExternalDNS to manage (required when dns_provider is 'aws')"
  default     = ""

  validation {
    condition     = var.dns_provider != "aws" || var.zone_id_filter != ""
    error_message = "zone_id_filter is required when dns_provider is 'aws'."
  }
}
...
variable "zone_type" {
  ...
  validation {
    condition     = var.dns_provider != "aws" || (var.zone_type != "" && contains(["public", "private"], lower(var.zone_type)))
    error_message = "When dns_provider is 'aws', zone_type must be 'public' or 'private'."
  }
}
...
variable "oci_compartment_ocid" {
  type        = string
  description = "The OCI compartment OCID where the DNS zones are located (required when dns_provider is 'oci')"
  default     = null

  validation {
    condition     = var.dns_provider != "oci" || var.oci_compartment_ocid != null
    error_message = "oci_compartment_ocid is required when dns_provider is 'oci'."
  }
}

variable "oci_region" {
  type        = string
  description = "The OCI region for workload identity configuration (required when dns_provider is 'oci')"
  default     = null

  validation {
    condition     = var.dns_provider != "oci" || var.oci_region != null
    error_message = "oci_region is required when dns_provider is 'oci'."
  }
}
